...
const buildAssetFilters = (
  ...
  if (query?.condition) {
    filters.push(eq(assets.condition, query.condition));
  }

  if (query?.category) {
    filters.push(eq(assets.category, query.category));
  }

  if (query?.accountingTreatment) {
    filters.push(eq(assets.accountingTreatment, query.accountingTreatment));
  }
